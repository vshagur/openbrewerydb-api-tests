...
class TestBackupDB:
    ...
    @pytest.fixture(scope='class',
                    params=range(1, constants.NUMBER_DB_RECORDS + 1),
    ...
    def test_brewery_type_fields(self, item):
        ...
        assert value == '' or value in constants.BREWERY_TYPES

    def test_postal_code_fields(self, item):
        """check postal_code field"""
        value = item['postal_code']
        assert value == '' or re.match(constants.POSTAL_CODE_REGEX, value)

    def test_name_fields(self, item):
        """check name field"""
        value = item['name']
        assert constants.MIN_LENTH_NAME_FIELD <= len(value) <= \
               constants.MAX_LENTH_STRING_FIELD

    def test_city_fields(self, item):
        """check city field"""
        value = item['city']
        assert re.match(constants.CITY_REGEX, value)

    def test_street_fields(self, item):
        """check street field"""
        value = item['street']
        assert constants.MIN_LENTH_STRING_FIELD <= len(value) <= \
               constants.MAX_LENTH_STRING_FIELD

    def test_state_type_fields(self, item):
        """check state field"""
        value = item['state']
        assert value == '' or value in constants.USA_STATES

    def test_country_type_fields(self, item):
        """check country field"""
        value = item['country']
        assert value == '' or value in constants.COUNTRIES

    def test_longitude_fields(self, item):
        """check longitude field"""
        value = item['longitude']
        assert value == '' or constants.MIN_LONGITUDE_VALUE <= float(value) <= \
                              constants.MAX_LONGITUDE_VALUE

    def test_latitude_fields(self, item):
        """check latitude field"""
        value = item['latitude']
        assert value == '' or constants.MIN_LATITUDE_VALUE <= float(value) <= \
                              constants.MAX_LATITUDE_VALUE

    def test_website_url_fields(self, item):
        """check website_url field"""
        value = item['website_url']
        assert value == '' or re.match(constants.WEBSITE_REGEX, value)

    def test_updated_at_fields(self, item):
        """check updated_at field"""
        value = item['updated_at']
        try:
            time.strptime(value, constants.DATETIME_PATTERN_DB)
        except:
            assert False
